...
<ul>
	...
	<li>스크립트릿에 많이 사용되는 for,if 등을 태크로 제공합니다</li>
	<li>jstl 라이브러리 필요합니다.</li>
	<li>el은 expression language. 기호는 $&#123;&#125; 사용합니다.
	<%-- <%= %>  --%>기호 대신에 사용합니다. 애트리뷰트 getAttribute 메소드 동작 실행 후 출력</li>
	<li>jstl과 el은 함꼐 사용됩니다. (el은 라이브러리 추가 없이 사용합니다.)</li>
</ul>
	<h4>자바 변수처럼 값을 저장하기</h4>
	<c:set var="age" value="10"/>
	age: ${age} <br>				<!--XSS 해킹공격에 취약한 형식  -->
	age: <c:out value="${age }"/>	<!--el 출력할때 사용합니다.  엄격한 형식을 적용하는 방법-->
	<hr>
	<!--  -->
	...
	<c:choose>
		...
		<c:when test="${age < 23 }">	<!-- 첫번쨰 조건이  거짓일때만 실행합니다. -->
	...
	<c:out value="${i }"></c:out>
	<c:out value="${i*10 }"></c:out><br>
	</c:forEach>
	<c:forEach var="x" begin="10" end="30" step="2" varStatus="status"><!-- varStatus는 현재상태 저장할 변수 -->
	<c:out value="${x }"></c:out>,index=
	<c:out value="${status.count }"></c:out><br> <!--varStatus의 count 속성은 1부터 시작하여 몇번째 값인지  -->
	</c:forEach>
	
	<h4>forTokens</h4>
	<c:set var="twice" value="사나,나연,다현,모모"></c:set>
	<ul>
