...
@Getter
public class PageVo {	//게시판의 페이지번호 목록을 만들기 위해 필요한 값들을 모두 저장합니다. (endPage,endNo는 생략가능)
	private int currentPage;		//현재 페이지
	private int totalCount;			//글의 전체 갯수 dao에 매소드 작성
	private int pageSize;			//한개페이지에 보여줄 글 갯수
	
	private int totalPage;			//전체 페이지 갯수 totalCount와 PageSize로 계산합니다.
	private int startPage;			//현재 페이지값을 이용해서 계산합니다.
	...
	public PageVo(int currentPage, int totalCount, int pageSize) {   //외부(비지니스로직)에서 결정하고 전달되는값.
		...
		//totalCount = 367개, pageSize=0, totalPage =37
		//totalCount = 7개, pageSize=20, totalPage =1
		//totalCount = 200개, pageSize=20, totalPage =10
		totalPage = (int)Math.ceil((double)totalCount/pageSize);//(totalCount-1)/pageSize + 1;대신에 자리올림 메소드 ceil사용
